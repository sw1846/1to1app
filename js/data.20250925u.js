...
/* [FIX: AppData hoist]
   原因: __root 変数が本ファイル後半で定義される前に `__root.AppData` に代入しており、
        実行時に "__root が未定義" -> "Cannot read properties of undefined (reading 'AppData')" を引き起こしていました。
   対応: IIFE 冒頭で __root を確実に定義し、同時に AppData 器を初期化してから以降の処理で利用します。
   影響範囲: グローバル汚染は従来通り window.AppData を前提。既存 API 署名・関数名は変更なし。
*/
...
async function getFileIdInFolder(fileName, parentFolderId){
  try{
    ...
    const id = await driveFindChildByName(parentFolderId, String(fileName), null);
    return id || null;
  }catch(e){
