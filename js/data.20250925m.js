...
(function(global){
  ...
  const scopes = (APP_CONFIG.SCOPES || '')
    .replace(/\u3000/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  const cid = (APP_CONFIG.GOOGLE_CLIENT_ID || '').trim();
  if (!cid) { console.error('[GIS] client_id missing'); alert('config.js の GOOGLE_CLIENT_ID が空です'); return; }
  if (!scopes) { console.warn('[GIS] scopes empty; fallback to drive.metadata.readonly'); }
  try {
    window.tokenClient = google.accounts.oauth2.initTokenClient({
      client_id: cid,
      scope: scopes || 'https://www.googleapis.com/auth/drive.metadata.readonly',
      use_fedcm_for_prompt: true,
      callback: (resp) => { console.log('[GIS] token resp:', resp); if (resp && resp.access_token) { setStatus('サインイン済み'); } },
      error_callback: (err) => { console.error('[GIS] error:', err); try{ alert('[GIS ERROR] ' + JSON.stringify(err)); }catch(e){ alert('[GIS ERROR] ' + (err && (err.error||err.type||'unknown'))); } },
      prompt: 'consent'
    });
  } catch (e) {
    console.error('[GIS] initTokenClient exception:', e);
    alert('initTokenClient 例外: ' + e.message);
  }
});
...
;(function(g){ g.__BUILD_ID__='2025-09-25M'; })(window);
